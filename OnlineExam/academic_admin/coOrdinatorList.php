...
<html lang="en">
<head>
    <title>::..Coordinator List|Academic Admin..::</title>
    <meta charset="utf-8">
...
<div class="row w-100 m-auto" >
    ...
    <div class="col-lg-9 float-left col px-5 py-2 main mr-5 my-4 border rounded" style="background-color:#f5f5f5;">
        <div class="page-header text-center" >
            <h3 class="mt-3">Coordinator List</h3>
        </div>
        <hr class="pb-1">
            ...
            <div class="col-md-12 mx-auto">
                <form class="form-inline ml-4" role="form">
                    <div class="form-group">
                        ...
                        <select class="form-control mr-3 my-2" id="filterBy">
                    ...
                    <button class="btn btn-primary mx-2 my-2"  id="search"><span class="fa fa-search" aria-hidden="true"></span>Search</button>
                    <a  href="addCoordinator.php" class="btn btn-success mx-2 my-2"><span class="fa fa-user-plus" aria-hidden="true"></span> Add Coordinator</a>
                </form>
            </div>
        </div>
        <hr class="pb-1">
        <div class="container rounded px-3" style="box-shadow: 1px 1px 10px black; margin-bottom: 30px; height: 800px;overflow-y: scroll;">
            <table class="table table-lg table-bordered mt-4 mb-lg-5">
                ...
                <tbody class="text-center">
                    <?php
                    $co_list_query = "SELECT * FROM co_ordinator";
                    $co_list_exists = $conn->query("$co_list_query");
                    $co_list_count = mysqli_num_rows($co_list_exists);

                    if($co_list_count >0) {
                        $count = 1;
                        while ($co_list_row = mysqli_fetch_array($co_list_exists)) {
                            $co_user_id = $co_list_row['user_id'];

                            $user_query = "SELECT * FROM user WHERE user_id = '$co_user_id'";
                            $user_exists = $conn->query("$user_query");
                            $user_row = mysqli_fetch_array($user_exists);
                            $co_full_name = $user_row['full_name'];

                            echo '<tr>
                                <td scope="col">' . $count . '</td>
                                <td scope="col">' . $co_list_row['co_id'] . '</td>
                                <td scope="col">' . $co_full_name . '</td>
                                <td scope="col">' . $co_list_row['faculty'] . '</td>
                                <td scope="col">' . $co_list_row['department'] . '</td>
                                <td scope="col" class="py-0">
                                    <a href="../apps/controller/acaAdminListController.php?delete='.$co_user_id.'" class="btn btn-sm btn-danger fa fa-trash" title="Delete Coordinator"></a>
                                </td>
                                </td>
                            </tr>';
                            $count = $count + 1;
